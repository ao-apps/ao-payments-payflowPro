...
public class PayflowPro implements MerchantServicesProvider {
  ...
  private AuthorizationResult authorizeOrSale(TransactionRequest transactionRequest, CreditCard creditCard, String trxType) {
    ...
    try {
      ...
      final PayflowConnectionData connectionData = new PayflowConnectionData(
      ...
      if (orderNumber != null && orderNumber.length() > 0) {
        invoice.setCustRef(orderNumber);
      }
      ...
      if (taxAmount != null) {
        invoice.setTaxAmt(new Currency(taxAmount.doubleValue(), transactionRequest.getCurrency().getCurrencyCode()));
      }
      invoice.setTaxExempt(transactionRequest.getTaxExempt() ? "Y" : "N");
      if (shippingAmount != null) {
        invoice.setFreightAmt(new Currency(shippingAmount.doubleValue(), transactionRequest.getCurrency().getCurrencyCode()));
      }
      if (dutyAmount != null) {
        invoice.setDutyAmt(new Currency(dutyAmount.doubleValue(), transactionRequest.getCurrency().getCurrencyCode()));
      }
      ...
      if (state != null && state.length() > 0) {
        if (state.length() == 2) {
          ...
          logger.log(Level.WARNING, "PayflowPro: state is not two-digits, and no automatic conversion has been implemented, not sending STATE", state);
        }
      }
      String zip = creditCard.getPostalCode();
      if (zip != null && zip.length() > 0) {
        billTo.setZip(CreditCard.numbersOnly(zip));
      }
      String cardCountryCode = creditCard.getCountryCode();
      if (cardCountryCode != null && cardCountryCode.length() > 0) {
        billTo.setBillToCountry(cardCountryCode);
      }
      String companyName = creditCard.getCompanyName();
      if (companyName != null && companyName.length() > 0) {
        billTo.setCompanyName(companyName);
      }
      ...
      if (shippingStreetAddress1 != null && shippingStreetAddress1.length() > 0) {
        shipTo.setShipToStreet(shippingStreetAddress1);
      }
      ...
      if (shippingCountry != null && shippingCountry.length() > 0) {
        if ("US".equals(shippingCountry)) {
          ...
          if (logger.isLoggable(Level.WARNING)) {
            logger.warning("shippingCountry unknown, not sending SHIPTOCOUNTRY: " + shippingCountry);
          }
        }
      }
      ...
      if (customerIp != null && customerIp.length() > 0) {
        customerInfo.setCustIP(customerIp);
      }
      String customerId = creditCard.getCustomerId();
      if (customerId != null && customerId.length() > 0) {
        customerInfo.setCustCode(customerId);
      }
      ...
      if (orderNumber != null && orderNumber.length() > 0) {
        ...
        requestId = PayflowUtility.getRequestId();
      }
    ...
    if (result == 0) {
      ...
      if (result == 12) {
        ...
        declineReason = AuthorizationResult.DeclineReason.MAX_SALE_EXCEEDED;
      } else if (result == 112) {
